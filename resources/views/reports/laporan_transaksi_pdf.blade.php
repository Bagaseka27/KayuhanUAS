...
<head>
    ...
    <style>
        ...
        .summary { margin-bottom: 20px; font-size: 11px; border: 1px solid #ccc; padding: 10px; }
...
<body>
    <div class="header">
        ...
        <p>
            ...
            Periode: {{ \Carbon\Carbon::parse($fromDate)->format('d F Y') }} s/d {{ \Carbon\Carbon::parse($toDate)->format('d F Y') }}
        </p>
    </div>
            ...
            <tr>
                ...
                <td>
                    @foreach($trx->detailtransaksi as $detail)
                        {{ $detail->JML_ITEM }}x {{ $detail->menu->NAMA_PRODUK ?? 'Produk Dihapus' }} <br>
                    @endforeach
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="6" style="text-align: center;">Tidak ada data transaksi yang ditemukan dalam periode ini.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
