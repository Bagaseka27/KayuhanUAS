...
<div class="container-fluid py-4">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4">
        ...
        <div class="mt-3 mt-md-0">
            <a href="{{ route('gaji.daftarPengambilan') }}" class="btn btn-success px-4 py-2 me-2 fw-bold rounded-3 shadow-sm">
                <i class="fas fa-money-bill-wave me-2"></i> Pengambilan Gaji
            </a>
            <a href="{{ route('gaji.daftarPenyimpanan') }}" class="btn btn-outline-success px-4 py-2 fw-bold rounded-3">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-3">
            <form action="{{ route('gaji.daftarPengambilan') }}" method="GET" class="row g-3">
                <div class="col-md-4">
            ...
            <div class="table-responsive">
                <table class="table custom-table align-middle mb-0">
                    <thead class="bg-light text-secondary text-uppercase small fw-bold">
                        <tr>
                            <th class="py-3 ps-4">Tanggal Pengambilan</th>
                            <th>Nama Karyawan</th>
                            <th>Email</th>
                            <th>Nominal Ambil</th>
                            <th>Status</th>
                    ...
                    <tbody class="bg-white">
                        ...
                        <tr>
                            <td class="py-3 ps-4 text-muted">{{ \Carbon\Carbon::parse($p->TANGGAL_PENGAMBILAN)->format('d/m/Y') }}</td>
                            <td class="fw-bold text-dark">{{ $p->karyawan?->NAMA ?? 'N/A' }}</td>
                            ...
                            <td class="text-center pe-4">
                                @if($p->STATUS === 'menunggu')
                                    <button type="button" class="btn btn-primary btn-sm px-3 rounded-pill fw-bold py-1.5 shadow-sm" data-bs-toggle="modal" data-bs-target="#terimaModal{{ $p->id }}">
                                        <i class="fas fa-check-circle me-1"></i> Proses Gaji
                                    </button>
                                    <button type="button" class="btn btn-outline-danger btn-sm px-3 rounded-pill fw-bold py-1.5 ms-1" data-bs-toggle="modal" data-bs-target="#tolakModal{{ $p->id }}">
                                        <i class="fas fa-times-circle me-1"></i> Tolak
                                    </button>
                                @else
                                    <button type="button" class="btn btn-outline-info btn-sm px-3 rounded-pill fw-bold py-1.5" data-bs-toggle="modal" data-bs-target="#detailModal{{ $p->id }}">
                        ...
                        <!-- Modal Terima -->
                        <div class="modal fade" id="terimaModal{{ $p->id }}" tabindex="-1">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content border-0 rounded-4 shadow">
                                    <div class="modal-header text-white py-3" style="background-color: #003d2e;">
                                        <h5 class="modal-title fw-bold">Proses & Setujui Pengambilan Gaji</h5>
                                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                    </div>
                                    <form action="{{ route('gaji.terimaPengambilan', $p->id) }}" method="POST">
                                        @csrf
                                        <div class="modal-body p-4">
                                            <p class="mb-2"><strong>Karyawan:</strong> {{ $p->karyawan?->NAMA }}</p>
                                            <p class="mb-2"><strong>Email:</strong> {{ $p->EMAIL }}</p>
                                            <p class="mb-3"><strong>Nominal Ambil:</strong> Rp {{ number_format($p->NOMINAL, 0, ',', '.') }}</p>
                                            
                                            <div class="alert alert-info border-0 rounded-3 small mb-3">
                                                <i class="fas fa-info-circle me-2"></i>
                                                Sisa dari total gaji barista yang belum diambil akan otomatis dialihkan ke tabungan (Penyimpanan Gaji) dengan status langsung disetujui.
                                            </div>

                                            <div class="mb-3">
                                                <label for="catatan_terima" class="form-label fw-bold small text-secondary">Catatan Admin (Opsional)</label>
                                                <textarea name="CATATAN_ADMIN" class="form-control bg-light border-0 rounded-3" rows="3" placeholder="Contoh: Gaji sudah disiapkan secara tunai/transfer..."></textarea>
                                            </div>
                                        </div>
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content border-0 rounded-4 shadow">
                                    <div class="modal-header text-white py-3 bg-danger">
                                        <h5 class="modal-title fw-bold">Tolak Pengambilan Gaji</h5>
                                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                    </div>
                                    <form action="{{ route('gaji.tolakPengambilan', $p->id) }}" method="POST">
                                        @csrf
                                        <div class="modal-body p-4">
                                            <p class="mb-2"><strong>Karyawan:</strong> {{ $p->karyawan?->NAMA }}</p>
                                            <p class="mb-3"><strong>Nominal Ambil:</strong> Rp {{ number_format($p->NOMINAL, 0, ',', '.') }}</p>
                                            <div class="mb-3">
                                                ...
                                                <textarea name="CATATAN_ADMIN" class="form-control bg-light border-0 rounded-3" rows="3" placeholder="Berikan alasan penolakan agar barista mengetahuinya..." required></textarea>
                                            </div>
                                        </div>
                                        <div class="modal-footer border-0 bg-light p-3 rounded-bottom-4">
                                            <button type="button" class="btn btn-secondary px-4" data-bs-dismiss="modal">Batal</button>
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content border-0 rounded-4 shadow">
                                    <div class="modal-header text-white py-3 bg-info">
                                        <h5 class="modal-title fw-bold">Detail Pengambilan Gaji</h5>
                                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                    </div>
                                    <div class="modal-body p-4">
                                        <div class="d-flex justify-content-between border-bottom py-2">
                                            <strong>Karyawan:</strong> <span>{{ $p->karyawan?->NAMA }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between border-bottom py-2">
                                            <strong>Email:</strong> <span>{{ $p->EMAIL }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between border-bottom py-2">
                                            <strong>Nominal Ambil:</strong> <span>Rp {{ number_format($p->NOMINAL, 0, ',', '.') }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between border-bottom py-2">
                                        ...
                                        <div class="d-flex justify-content-between border-bottom py-2">
                                            <strong>Catatan Admin:</strong> <span>{{ $p->CATATAN_ADMIN ?? '-' }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between border-bottom py-2">
                                            <strong>Diproses Oleh:</strong> <span>{{ $p->adminProses?->NAMA ?? $p->DIPROSES_OLEH ?? '-' }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between py-2">
                                            <strong>Waktu Proses:</strong> <span>{{ $p->TANGGAL_DIPROSES ? \Carbon\Carbon::parse($p->TANGGAL_DIPROSES)->format('d/m/Y H:i') : '-' }}</span>
                                        </div>
                                    </div>
                                    <div class="modal-footer border-0 bg-light p-3 rounded-bottom-4">
                        ...
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4">
                                <i class="fas fa-inbox me-2"></i> Tidak ada data pengajuan pengambilan gaji.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
